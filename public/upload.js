...
import { setDoc, getDocs, doc , collection } from "https://www.gstatic.com/firebasejs/9.6.1/firebase-firestore.js";
...
document.getElementById('uploadForm').addEventListener('submit', async (e) => {
    ...
    // 파일 업로드 처리
    Array.from(files).forEach((file, index)=> {
        const storageRef = ref(storage, `uploads/${postId}/${file.name}`); // postId 폴더 생성
        const uploadTask = uploadBytes(storageRef, file).then(snapshot => {
            return getDownloadURL(snapshot.ref).then(downloadURL => {
                return { index: index, fileName: file.name, url: downloadURL, type: file.type };
            });
        });
        //uploadPromises.push(uploadTask);
        uploadPromises[index] = uploadTask;  // 인덱스에 따라 업로드 작업 할당
    });

    // 현재 posts의 문서 수를 가져와 +1 한 값을 orders로 설정
    const postsCount = await getPostsCount(db);
    const orderValue = postsCount + 1;    
